(function ($) {
    ...
    $.fn.init2048 = function (_options) {
        ...
        function resetGame() {
            ...
            content = $('<div>').addClass('container_2048').appendTo(holder);
            for (var i = 0; i < 4; i++) {
                for (var j = 0; j < 4; j++) {
                    //Reset matrix
                    matrix[i * 4 + j] = {
        ...
        function createBox(value) {
            ...
            var newBox = $('<div>').addClass('box2048').attr({
        ...
        function combineBox(source, target, value) {
			...
			currentHight = Math.max(currentHight,_value);		
        }
        ...
        function gameRun(dir) {
            ...
            if (gameOver()) {
                isGameOver = true;
//				sfbg_2048_show_reward_confirm(currentHight);
				sbf_sf_show_reward_confirm(currentHight,sfbg_2048_show_reward,'2048')
            }
        }
...
//TROF START		
				if( ( ( typeof(sfbs_get_active_tab) == 'function')
				 && (sfbs_get_active_tab() != '2048') )
				 || ($( document.activeElement ).is('input'))
				 )
				 {
//console.log('2048 - not our tab for key events!');
					return true;
				 }
//TROF END				
                if (!isGameOver) {
